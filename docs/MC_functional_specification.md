...
`일반 사용자`는 생성된 `과`에 가입 신청을 할 수 있으며 신청이 승인되면 `과 사용자`가 된다. `과 사용자`는 `과 보드`를 이용하거나 `모임`에 가입 신청할 수 있다. `과 보드`는 `과`에 가입된 모든 사용자가 이용할 수 있는 게시판에 대한 컴포넌트다. `모임`은 `과 관리자`에 의해 생성되는 컴포넌트로 과 내 단체를 위한 기능을 제공한다. '과 관리자 신청'을 통해 `과`를 생성한 사용자는 `과 관리자`가 된다. `과 관리자`는 `과 사용자 관리`, `과 보드 관리`, `모임 관리` 기능을 수행할 수 있다.

`모임`을 생성한 `과 관리자`는 `모임 관리자` 권한을 부여할 수 있다. `모임 관리자`는 `모임 사용자 관리`, `모임 보드 관리` 기능을 수행할 수 있다. `모임 보드`는 `모임`에 가입된 사용자가 이용할 수 있는 게시판에 대한 컴포넌트다.
