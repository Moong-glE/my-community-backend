...
My-community는 `과` 단위로 구성된다. 관리자는 본 어플리케이션의 개인 계정 생성 후 '과 관리자 신청'을 통해 `과`를 생성할 수 있으며 `과 관리자` 권한을 획득한다. `과 관리자`는 `과 보드`, `모임`, `모임 관리자`에 대해 관리할 수 있다. `과 보드`는 `과`에 가입된 모든 사용자가 이용할 수 있는 게시판에 대한 컴포넌트다. 

`모임`은 `과 관리자`에 의해 생성되는 컴포넌트로 과 내 단체를 위한 기능을 제공한다. `모임 보드`는 `모임`에 가입된 사용자가 이용할 수 있는 게시판에 대한 컴포넌트다. `모임 관리자`는 `모임 보드`를 관리할 수 있다. 
